...
#ifdef _WIN32
  vector<string> v{ "  Buenos dÌas ", " sean dados !", " a todo el mundo\n" };
#elif __linux__
...
void test161()
{
  ...
  /* Compiler is gcc/g++ */
    ...
    // const std::chrono::year_month_day ymd{std::chrono::floor<std::chrono::days>(now)};
     ...
     std::cout << "A√±o: " << static_cast<int>(d.year()) << ", "
                 "Mes: " << static_cast<unsigned>(d.month()) << ", "
                 "D√≠a : " << static_cast<unsigned>(d.day()) << "\n";
...
void programaInvocadoSinParametros( int argc, char *argv[])
{
	...
	escribeNumeroAleatorio(cout, "N˙mero aleatorio alea:");
...
int main(int argc, char* argv[])
{
	...
	{	
      ...
      // cout << "Uso: " << argv[0] << " n˙mero" << std::endl;
	}
	else
	{
		cout << "La raÌz cuadrada de " << argv[1] << " es " << std::sqrt (std::stol(argv[1])) << endl;
	}
}
